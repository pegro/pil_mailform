...
require_once(ExtensionManagementUtility::extPath('pil_mailform') . 'phpmailer/class.phpmailer.php');

class tx_pilmailform_pi1 extends TYPO3\CMS\Frontend\Plugin\AbstractPlugin
{
    ...
    function main($content, $conf)
    {
        ...
        if (ExtensionManagementUtility::isLoaded('sr_freecap')) {
            $this->freeCap = GeneralUtility::makeInstance('SJBR\\SrFreecap\\PiBaseApi');
        }
        if ($this->init()) {
    ...
    function init()
    {
        ...
        foreach ($piFlexForm['data'] as $sheet => $data) {
            foreach ($data as $lang => $value) {
                foreach ($value as $key => $val) {
                    $this->localconf[$key] = !empty($this->localconf[$key]) ? $this->localconf[$key] : trim($this->pi_getFFvalue($piFlexForm,
                        $key, $sheet));
                }
            }
        }
        ...
        if ($this->localconf['useLL'] == 1) {
            $this->LOCAL_LANG = GeneralUtility::readLLfile($this->localconf['LLFile'], $this->LLkey);
        ...
        if (!strpos($this->cObj->getSubpart($this->template, '###TMAIL_FORM###'),
                '[text][email]') && ($this->localconf['overrideFromHeader'] != 0 || $this->localconf['overrideReplyToHeader'] != 0)
        ) {
    ...
    function get_mailform()
    {
        ...
        if ($this->localconf['typeofRecipient'] == 1) {
            $subpart = $this->cObj->substituteMarkerArray($subpart,
                array('###MULTI_RECIPIENT_OPTIONS###' => $this->get_multi_recipient()));
        }
    ...
    function load_static_markers(&$markers)
    {
        ...
        $markers['###_DATE###'] = strftime($this->localconf['dateMarkerFormat'],
            time()); // Current date in the format YY-MM-DD (allows natural sorting in the mail client)
        ...
        if ($this->localconf['useLL'] == 1) {
            if (array_key_exists($this->LLkey, $this->LOCAL_LANG)) {
                foreach ($this->LOCAL_LANG[$this->LLkey] as $k => $v) {
                    //Patched by HDNET
                    $markers['###_LL_' . strtoupper($k) . '###'] = $this->pi_getLL($k, '', true);
                }
            }
        }
    ...
    {
        GeneralUtility::sysLog($str, $this->extKey);
    }
            ...
            foreach ($field as $k => $v) {
                if (is_array($v)) {
                    ...
                    switch ($type) {
                        ...
                        case 'checkbox':
                            $markers['###' . strtoupper($k) . '_VAL###'] = !empty($v) || strlen($v) >= 1 ? htmlspecialchars($v) : '';
                            $markers['###' . strtoupper($k) . '_' . strtoupper(preg_replace('/[^a-z0-9]/i', '_',
                                $v)) . '_VAL###'] = !empty($v) || strlen($v) >= 1 ? 'checked="checked"' : '';
                        ...
                        case 'select':
                            $markers['###' . strtoupper($k) . '_VAL###'] = !empty($v) || strlen($v) >= 1 ? htmlspecialchars($v) : '';
                            $markers['###' . strtoupper($k) . '_' . strtoupper(preg_replace('/[^a-z0-9]/i', '_',
                                $v)) . '_VAL###'] = !empty($v) || strlen($v) >= 1 ? 'selected="selected"' : '';
                            $markers['###' . strtoupper($k) . '_ERR###'] = ($requiredFields[$k] && (empty($v) || strlen($v) <= 0)) ? $this->localconf['errorSubstitution'] : '';
                            $markers['###' . strtoupper($k) . '_ERR_TXT###'] = $reqVal['is_error'] ? $reqVal['error_str'] : '';
                            break;
                        case 'file':
    ...
    function validate_field($fkey, $fval, $reqVal, &$markers)
    {
        ...
        if (is_array($reqVal)) {
            foreach ($reqVal as $k => $v) {
                $v['estr'] = $v['ekey'] == 'lang' ? $this->pi_getLL(trim($v['estr']), 'UNKNOWN LANGUAGE MARKER',
                    true) : $v['estr'];
                switch ($v['eval']) {
                    case 'notEmpty':
                        if (empty($fval)) {
            ...
            switch ($dvc['method']) {
                case 'plain':
                    ...
                    if ($dvc['proc'] == 'text') {
                        ...
                        $markers['###' . strtoupper($dvc['fields']) . '###'] = $this->pi_getLL($dvc['pparm'],
                            'UNKNOWN LANGUAGE MARKER', true);
                    }
                    break;
                case 'db':
                    $subparts = $this->trim_explode(':', $dvc['pparm'], true, 3);
                    $this->array_append($this->trim_explode(',', $subparts[1]),
                        $queries[$dvc['proc']][$subparts[0]][$subparts[2]]['fields']);
                    $this->array_append($this->trim_explode(',', $dvc['fields']),
                        $queries[$dvc['proc']][$subparts[0]][$subparts[2]]['markers']);
                    break;
                default:
            }
        }
            ...
            foreach ($queryArr as $table => $whereclause) {
                ...
                foreach ($whereclause as $wc => $parms) {
                    ...
                    if (count($parms['fields']) != count($parms['markers'])) {
                        $this->error_handler('Inconstistent number of fields/values: ' . implode(',',
                                $parms['fields']) . ' => ' . implode(',', $parms['markers']));
        ...
        if ($wc[0] == '*') {
            ...
            if (count($parts) == 2) {
                ...
                if (array_key_exists($parts[0], $fn[$tbl])) {
                    ...
                    if (preg_match('/^(tinyint|smallint|int|mediumint|bigint|float|double|decimal|numeric)/i',
                            $fn[$tbl][$parts[0]]) && !is_numeric($parts[1])
                    ) {
